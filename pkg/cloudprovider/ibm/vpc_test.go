...
func TestListSpotInstancesPagination(t *testing.T) {
	starts := []string{}
	mock := &mockVPCClient{
		listInstancesFunc: func(options *vpcv1.ListInstancesOptions) (*vpcv1.InstanceCollection, error) {
			if options.AvailabilityClass == nil || *options.AvailabilityClass != vpcv1.InstanceAvailabilityPrototypeClassSpotConst {
				t.Fatalf("expected spot availability class, got %v", options.AvailabilityClass)
			}
			if options.Start == nil {
				starts = append(starts, "")
				return &vpcv1.InstanceCollection{
					Instances: []vpcv1.Instance{{ID: core.StringPtr("spot-page-one")}},
					Next:      &vpcv1.PageLink{Href: core.StringPtr("https://example.test/v1/instances?start=spot-page-two")},
				}, nil
			}
			starts = append(starts, *options.Start)
			return &vpcv1.InstanceCollection{Instances: []vpcv1.Instance{{ID: core.StringPtr("spot-page-two")}}}, nil
		},
	}

	instances, err := NewVPCClientWithMock(mock).ListSpotInstances(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	if len(instances) != 2 {
	...
	if !reflect.DeepEqual(starts, []string{"", "spot-page-two"}) {
		t.Fatalf("expected page starts [\"\" \"spot-page-two\"], got %v", starts)
	}
}
