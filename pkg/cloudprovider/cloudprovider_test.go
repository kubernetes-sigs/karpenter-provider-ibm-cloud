...
func TestCloudProvider_IsDrifted_SubnetCheckErrors_WhenNoSubnetsDiscovered(t *testing.T) {
	ctx := context.Background()
	scheme := getTestScheme()

	nodeClass := getTestNodeClass()
	nodeClass.Status.SelectedSubnets = nil // empty => should error if subnet annotation exists
	...
	_, err := cp.IsDrifted(ctx, nodeClaim)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "no subnets discovered")
}
